...
from tap_azure_cloud_storage.discover import discover_streams, discover_schema, load_metadata
...
class TestDiscovery(unittest.TestCase):
    ...
    def test_discover_streams_includes_valid_tables(self):
        ...
        with patch('tap_azure_cloud_storage.azure_storage.get_sampled_schema_for_table', return_value=mock_schema):
    ...
    def test_discover_streams_skips_tables_without_files(self):
        ...
        with patch('tap_azure_cloud_storage.azure_storage.get_sampled_schema_for_table', side_effect=mock_get_schema):
            streams = discover_streams(config)
    ...
    def test_discover_streams_logs_skipped_tables(self):
        ...
        config = {
            'tables': [
                {'table_name': 'skipped1', 'search_prefix': 'p1/', 'search_pattern': '.*', 'key_properties': []},
                {'table_name': 'skipped2', 'search_prefix': 'p2/', 'search_pattern': '.*', 'key_properties': []}
            ]
        }

        with patch('tap_azure_cloud_storage.azure_storage.get_sampled_schema_for_table', return_value=None):
            streams = discover_streams(config)
    ...
    def test_discover_streams_returns_valid_structure(self):
        """Test that discover_streams returns streams with correct structure"""
        config = {
            'tables': [
                {
                    'table_name': 'my_table',
        ...
        mock_schema = {
            ...
            'properties': {
                'id': {'type': 'integer'},
                'name': {'type': 'string'},
                '_sdc_source_container': {'type': ['null', 'string']},
                '_sdc_source_file': {'type': ['null', 'string']},
                '_sdc_source_lineno': {'type': ['null', 'integer']},
                '_sdc_extra': {'type': ['null', 'array', 'object']}
            }
        }
        ...
        # Check that stream has the required structure
        self.assertEqual(len(streams), 1)
        self.assertIn('schema', streams[0])
        self.assertIn('metadata', streams[0])
        schema_properties = streams[0]['schema']['properties']
        # Verify basic fields exist
        self.assertIn('id', schema_properties)
        self.assertIn('name', schema_properties)
